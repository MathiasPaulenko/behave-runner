...
from behave_runner.core.output import ensure_output_dir, find_latest_report
...
FormatType = Literal["console", "html", "md", "json", "xlsx", "pdf"]
...
def report_command(
    ...
    fmt: str = typer.Option(
        "console",
        "--format",
        help="Report format: console, html, md, json, xlsx, pdf.",
    ),
    output: Path | None = typer.Option(None, "--output", help="Output directory for reports."),
) -> None:
    ...
    if not check_optional("report", package, f"format {fmt}"):
        raise typer.Exit(2)

    if output is not None:
        ensure_output_dir(output)

    cmd = [f"behave-modern-{fmt}-report"]
    if output is not None:
...
def report_show(
    output: Path = typer.Option(Path("reports"), "--output", help="Directory containing reports."),
) -> None:
